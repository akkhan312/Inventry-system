...
export const listMappings = async (req: Request, res: Response) => {
    try {
        const mappings = await (prisma as any).barcodeMapping.findMany({
...
export const createMapping = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const mapping = await (prisma as any).barcodeMapping.create({
            data: {
                barcode,
                seriesNumber,
                item,
...
export const approveMapping = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;

        const mapping = await (prisma as any).barcodeMapping.update({
            where: { id },
            data: { status: 'mapped' }
        });
...
export const deleteMapping = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        await (prisma as any).barcodeMapping.delete({ where: { id } });
...
export const exportMappingsToExcel = async (req: Request, res: Response) => {
    try {
        const mappings = await (prisma as any).barcodeMapping.findMany({
            orderBy: { createdAt: 'desc' }
        });
