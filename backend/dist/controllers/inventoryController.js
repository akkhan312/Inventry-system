...
export const getStats = async (req, res) => {
    try {
        const [totalProducts, lowStockItems, outOfStockItems, categories, allProducts] = await Promise.all([
            ...
            prisma.product.findMany({ select: { quantity: true, purchasePrice: true } })
        ]);
...
export const getDashboardData = async (req, res) => {
    try {
        ...
        const [totalProducts, lowStockItems, outOfStockItems, categoryGroups, recentProducts, physicalCountSessions, allProducts] = await Promise.all([
            prisma.product.count(),
            prisma.product.count({ where: { status: 'low' } }),
            prisma.product.count({ where: { status: 'out' } }),
            ...
            prisma.physicalCountSession ? prisma.physicalCountSession.count() : Promise.resolve(0),
            prisma.product.findMany({ select: { quantity: true, purchasePrice: true, createdAt: true } })
        ]);
        const totalStockValue = calculateTotalStockValue(allProducts);
...
export const createProduct = async (req, res) => {
    try {
        const { name, sku, category, quantity, purchasePrice, salePrice, hsnCode, gstRate, unit, description, barcode, openingStock, minStock, reorderPoint, location, supplier, batchNumber, expiryDate } = req.body;
